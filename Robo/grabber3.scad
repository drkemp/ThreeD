...
module clamp1(ht) {
  ...
  difference() {
    ...
    translate([18,7,ht/2]) rotate([90,0,0]) cylinder(r=.75,h=5, $fn=50);
  }
...
//  difference() {
    gear(number_of_teeth=teeth,circular_pitch=200, clearance=.3,gear_thickness = gearthick,
           rim_thickness = gearthick, hub_thickness = gearthick, $fn=50);
...
module post(rout,rin,h) {
  difference() {
    cylinder(r=rout,h=h, $fn=50);
    translate([0,0,-1]) cylinder(r=rin,h=h+2, $fn=50); 
  }
}
...
module support1(rad,radin, t, clampt, xoff, yoff, shaftdia=7, shaftlen=7) {
  holedepth=21;
  difference() {
    translate([rad, yoff+3, 0]) rotate([90, 0, 0])  cylinder(r=rad, h=t, $fn=50);
    translate([rad, yoff+3, 0]) rotate([90, 0, 0])  cylinder(r=radin, h=t, $fn=50);
    ...
    translate([xoff, yoff, (-clampt / 2)-(holedepth+3)]) cylinder(r=2.4, h=holedepth, $fn=50);
    translate([xoff, yoff, (-clampt / 2-4)]) cylinder(r=1, h=holedepth, $fn=50);
    translate([2 * rad - xoff, yoff, (-clampt / 2)-(holedepth+3)]) cylinder(r=2.4, h=holedepth, $fn=50);
    translate([2 * rad - xoff, yoff, (-clampt / 2-4)]) cylinder(r=1, h=holedepth, $fn=50);

  }
  difference(){
    union() {
      translate([rad, -6.5, 0]) rotate([90, 0, 0])  cylinder(r=8, h=2.5, $fn=50);
      translate([rad-4,-7,(-clampt / 2)])  linear_extrude(height=10) 
           polygon(points=[[0,0],[8,0],[4,4]]);
    }
    translate([rad, 2, 0]) rotate([90, 0, 0])  cylinder(r=radin, h=30, $fn=50);
  }
  ...
  translate([rad,-8,0]) rotate([90,0,0])
     gear(number_of_teeth=12,circular_pitch=200, clearance=.3,gear_thickness = 5, rim_thickness=5,
            hub_thickness=shaftlen+6, hub_diameter= shaftdia,bore_diameter=radin*2, $fn=36);
...
module support2(rad,radin, t, clampt, xoff, yoff) {
  ...
  difference() {
    ...
    translate([xoff, yoff, (-clampt / 2-4)]) cylinder(r=1, h=holedepth, $fn=50);
    translate([2 * rad - xoff, yoff, (-clampt / 2)-(holedepth+3)]) cylinder(r=2.4, h=holedepth, $fn=50);
    translate([2 * rad - xoff, yoff, (-clampt / 2)-3]) cylinder(r=1, h=holedepth, $fn=50);
...
module bearingmount(id,od,len, flange, shaft, offset) {
  difference(){
    translate([0,0,0]) cylinder(r=flange/2,h=len+4, $fn=50);
    translate([0,0,-3]) cylinder(r=od/2,h=len+1, $fn=50);
    translate([0,0,0]) cylinder(r=id/2+2,h=len+6, $fn=50);
    translate([-offset,0,0]) cylinder(r=shaft/2,h=len+6, $fn=50);
    translate([-offset,0,len-4]) cylinder(r=5,h=len+6, $fn=50);
  }
  difference() {
    translate([0,0,-5]) cylinder(r=flange/2+2,h=7, $fn=50);
    translate([0,0,0 -6]) cylinder(r=flange/2,h=9, $fn=50);
  }
}
...
module armend(clampthick, bore,centeroffset, shaft, shaftoffset) {
  ...
  bod=22;
  bthick=7;
  translate([centeroffset,-14,clampthick/2]) rotate([90,0,0]) bearingmount(bid,bod,bthick, 44,shaft,shaftoffset);
...
module drivegear(clampthick, shaft, centeroffset,shaftoffsetx, shaftoffsety, shaftlen) {
  translate([centeroffset-shaftoffsetx,shaftoffsety,clampthick/2]) rotate([90,15,0])
     gear(number_of_teeth=12,circular_pitch=200, clearance=.3,gear_thickness = 3,
           rim_thickness = 3, hub_thickness = 3.75, hub_diameter= 10,bore_diameter=1, $fn=50);
    translate([centeroffset-shaftoffsetx,shaftoffsety,clampthick/2]) rotate([90,0,0]) cylinder(r=shaft/2,h=shaftlen, $fn=50);
}
...
module print() {
  ...
  clamp1(clampthick);
  translate([clampwidth,0,0]) clamp2(clampthick);
  translate([clampwidth*2+2,12,7]) rotate([-90,0,0]) support1(outdia/2,bore/2,supportthick,clampthick,13.5,4,7,16);
  translate([clampwidth*2+2+outdia,18,7]) rotate([90,180,0]) support2(outdia/2,9,supportthick,clampthick,13.5,4);

  translate([5,-0,4]) rotate([0,0,0])  drivegear(clampthick,4.8,18,13.5,-9.5,25);
  translate([20,-35,-9]) rotate([-90,0,0])   armend(clampthick,bore,18,5.2,13.5);
